...
public class UsachMock extends AbstractApiBean {
	...
	public Response activate(JsonObject jsonObject) throws IOException {
		...
		try {
			Response jsonApiAcademic = restUsachServiceBean.apiAcademic(rut);


		//	var jsonObject1 = (JSONObject )jsonApiAcademic.getEntity();
			final MultivaluedMap<String, String> formParams = jsonApiAcademic.readEntity(Form.class).asMap();
			System.out.println(formParams.size());
			if(((JsonObject) jsonApiAcademic.getEntity()).get("status").equals("ERROR")){
				return error(Response.Status.BAD_REQUEST, "Error: " + ((JsonObject) jsonApiAcademic.getEntity()).getString("message") );
			}
			var cxx =  restUsachServiceBean.createUser(null);
			return null;
		}catch (Exception e){
