...
//            logger.info(String.valueOf(r1.getStatus()));
    ...
    public static Response apiAcademic(String run) throws IOException {
        CloseableHttpClient httpClient = HttpClients.createDefault();
        try {
            ...
            if(statusCode == 404){
                //return error(Response.Status.BAD_REQUEST, "User Not Found");
                throw new RuntimeException("User Not Found");
            }
            if(statusCode ==500){
                String retSrc = "Error";
                JSONObject result = new JSONObject(retSrc);
                return error(Response.Status.BAD_REQUEST, "Error: Internal");
              //  return null;
            }
        } finally {
            httpClient.close();
